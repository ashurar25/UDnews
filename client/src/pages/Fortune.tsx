...
const FortunePage = () => {
  ...
  const generateFortune = (zodiac: ZodiacSign) => {
    if (!zodiac) return;
    
    setIsLoading(true);
    
    const today = new Date().toISOString().split('T')[0];
    const seed = hashCode(zodiac.thai + today);
    ...
    const random = (min: number, max: number) => {
        const rng = seededRandom(seed + min + max);
    ...
    const newFortune: FortuneReading = {
      ...
      luckyColors: getLuckyColors(zodiac.element, Math.floor(random(0, 2))),
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-4 bg-indigo-50 rounded-lg">
                        ...
                        <p className="font-sarabun text-indigo-600">{fortune.luckyColors.join(', ')}</p>
                    </div>
                    </div>
